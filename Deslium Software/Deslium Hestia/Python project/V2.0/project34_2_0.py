...
def main():  # основное тело программы
    def time():  # функция - генератор выбора времени для 16 пациентов на 8-ми часовой рабочий день
        while True:  # бесконечный цикл на выбор времени (для правильной работы if)
            time_of_receipt = random.choice(doc.self_times)  # выбираем случайное время из последовательности приёма доктора
            if time_of_receipt not in doc.self_busyTime:  # проверяем не занято ли сгенерированное время
                print("Доступное время: " + time_of_receipt)  # отображаем доступное время для записи
                ...
                if polvb == 1:  # если выбранное время устраивает - выполнить присовения, нет - вернуться к началу цикла
                    doc.self_busyTime.append(time_of_receipt)  # добавляем сгенерированное время в занятое время
                    return time_of_receipt  # возвращаем сгенерированное время
