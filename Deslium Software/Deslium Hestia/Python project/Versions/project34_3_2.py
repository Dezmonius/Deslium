...
def inbd():  # функция, добавляющие в список addp значения из функции forbd внутри класса доктор
    addp.extend(doc.forbd())  # походу ошибка где-то здесь


def bd_add(id, note):
    conn = sqlite3.connect('schedule.sqlite')
    c = conn.cursor()
    c.execute('CREATE TABLE sheet (id integer, note text)')
...
while True:
    ...
    try:
        ...
        if item_selected == 1:
            ...
            inbd()
            for x in addp:  # ТЕСТИРОВАНИЕ
                print(x)  # ТЕСТИРОВАНИЕ
